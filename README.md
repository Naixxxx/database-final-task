...
- Закрепить работу с PostgreSQL на наборе независимых учебных баз данных: транспорт, гонки, бронирования отелей и организационная структура.
- Отработать создание таблиц, заполнение их тестовыми данными и написание запросов для получения аналитических выборок.
- Использовать основные возможности SQL: `JOIN`, фильтрацию, группировку, агрегатные функции, `UNION ALL`, `CASE`, `STRING_AGG`, `COALESCE`, `WITH` и `WITH RECURSIVE`.
- На практике разобрать задачи с объединением данных из нескольких таблиц, подсчётом статистики, категоризацией записей и обходом иерархий сотрудников.
...
# Вход в psql (Если в вашей системе используется другой PostgreSQL-пользователь, замените `postgres` на имя своего пользователя)
psql -U postgres

# Запрос на создание базы данных (внутри psql)
CREATE DATABASE database_final_task;

# Инициализация подключения к базе данных (внутри psql)
...
потому что в ожидаемом выводе автомобиль со средней позицией ровно `3.0` учитывается в количестве автомобилей с низкой средней позицией. Исходя из этого я создал 2 файла с разными опциями task_5.sql, который соответствует условиям формально и task_5_second_version.sql, который соответствует ожидаемому выводу.
